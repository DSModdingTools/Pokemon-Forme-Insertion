...
def deconstruct_GARC(bindata, poke_edit_data):
    ...
    FATB_offset = FAT0_offset + from_little_bytes_int(
        bindata[FAT0_offset + 0x4 : FAT0_offset + 0x8]
    )
    ...
    for _ in range(file_count):
        ...
        data_offset = data_absolute_offset + from_little_bytes_int(
            bindata[FATB_offset + 0x4 : FATB_offset + 0x8]
        )
        # print(data_offset)
        # get length of current file
        file_length = from_little_bytes_int(
            bindata[FATB_offset + 0xC : FATB_offset + 0x10]
        )
...
def reconstruct_GARC(poke_edit_data, GARC_name):
    ...
    match GARC_name:
        case "personal":
            ...
            out_file = poke_edit_data.personal + [
                reduce(lambda i, j: i + j, poke_edit_data.personal)
            ]
        case "evolution":
    ...
    # FAT0 length
    temp[FAT0_offset + 0x4 : FAT0_offset + 0x8] = from_int_little_bytes(
        file_count * 4 + 0xC, 0x4
    )
    ...
    # padding (0x4)
        ...
        temp[FAT0_offset - 0xC : FAT0_offset - 0x8] = from_int_little_bytes(
            biggest_size_padding, 0x4
        )
        temp[FAT0_offset - 0x8 : FAT0_offset - 0x4] = from_int_little_bytes(
            biggest_size, 0x4
        )
...
def load_GARC(poke_edit_data, garc_path, target, gameassert):
    ...
    if os.path.exists(garc_path):
        ...
        try:
            file_array = deconstruct_GARC(
                binary_file_to_array(garc_path), poke_edit_data
            )
...
def rebuild_csv(poke_edit_data):
    ...
    for current_base_species in range(1, poke_edit_data.max_species_index + 1):
        ...
        species_indices_from_csv = find_rows_with_column_matching(
            poke_edit_data.master_list_csv, 2, current_base_species
        )
        ...
        temp_personal_pointer_garc: int = from_little_bytes_int(
            poke_edit_data.personal[current_base_species][0x1C:0x1E]
        )
        ...
        if temp_personal_pointer_garc in {"0", "", 0}:
            ...
            temp_personal_instance_garc = from_little_bytes_int(
                poke_edit_data.personal[current_base_species][0x20]
            )

        missing_personal_count = (
            temp_personal_instance_garc - temp_personal_instance_csv
        )
        missing_model_count = (
            species_model_count[current_base_species - 1] - temp_model_instance_csv
        )
        ...
        # at least one of model and Personal have more entries.
            ...
            for x in range(rows_to_add):
                ...
                if missing_model_count > x:
                    ...
                    temp_model_index = ""
                temp_csv_file.append(
                    [
                        temp_name,
                        temp_personal_instance_csv + x,
                        current_base_species,
                        temp_forme_personal_location,
                        temp_model_index,
                    ]
                )
...
def load_names_from_CSV(poke_edit_data, just_wrote=False):
    ...
    try:
        with open(
            poke_edit_data.csv_pokemon_list_path, newline="", encoding="utf-8-sig"
        ) as csvfile:
            ...
            for data_rows in loaded_csv_file:
                ...
                if (
                    data_rows[0].isdigit()
                    and data_rows[1].isdigit()
                    and data_rows[0] == data_rows[1]
                ):
                ...
                if not (data_rows[2] in {"", "NA"}):
                    ...
                    logger.warning(
                        "Entry without unique model file detected at Species Index-Personal Index-Name:"
                        + data_rows[0]
                        + "-"
                        + data_rows[1]
                        + "-"
            ...
            for offset in range(start_offset, len(poke_edit_data.model_header) - 2, 2):
                ...
                while True:
                    if isinstance(temp_loaded_csv[loaded_csv_row][4], int):
                        temp_loaded_csv[loaded_csv_row][5:7] = (
                            poke_edit_data.model_header[offset : offset + 2]
                        )
                        loaded_csv_row += 1
                        break
                    loaded_csv_row += 1
        ...
        if just_wrote:
            species_check = len(temp_base_species_list) - len(
                poke_edit_data.base_species_list
            )
            forme_check = len(temp_master_formes_list) - len(
                poke_edit_data.master_formes_list
            )
            model_check = len(temp_model_source_list) - len(
                poke_edit_data.model_source_list
            )

            if species_check < 0:
                logger.warning(
                    "The loaded CSV has fewer Pokemon base species than your game files. Something is very probably wrong unless you have successfully added new species to the game (in which case please submit a bug report so I can update). The Pokemon base species entries read from the CSV have NOT been loaded."
                )
                return poke_edit_data
            elif species_check > 0:
                logger.warning(
                    "The loaded CSV has more Pokemon base species than your game files. Something is very probably wrong. Please recheck your game files, the csv itself, and your settings. The Pokemon base species entries read from the CSV have NOT been loaded."
                )
                return poke_edit_data
            elif species_check == 0:
                logger.warning("Loading Pokemon Species List from CSV")
                poke_edit_data.base_species_list = temp_base_species_list.copy()

            if forme_check < 0:
                logger.warning(
                    "The loaded CSV might have fewer Forme entries than your game files. Unless you have not previously selected or initialized a csv for your game, or for whatever reason refreshed it to default, something might be wrong. Please double-check your file selections and settings. Will rebuild CSV file in memory from game files, if this is wrong, please exit without saving"
                )
                poke_edit_data = rebuild_csv(poke_edit_data)
            elif forme_check > 0:
                logger.warning(
                    "The loaded CSV has more total Forme entries than your game files. Something is wrong. Please double-check your file selections and settings. The Forme entries read from the CSV have NOT been loaded."
                )
                return poke_edit_data

            if forme_check == 0:
                logger.warning("Loading Formes List from CSV")
                poke_edit_data.master_formes_list = temp_master_formes_list.copy()

            if model_check < 0:
                logger.warning(
                    "The loaded CSV might have fewer total Model entries than your game files. Unless you have not previously selected or initialized a csv for your game, or for whatever reason refreshed it to default, something might be wrong. Please double-check your file selections and settings. Will attempt to update."
                )
                poke_edit_data = rebuild_csv(poke_edit_data)
                poke_edit_data = load_names_from_CSV(poke_edit_data, True)
            elif model_check > 0:
                logger.warning(
                    "The loaded CSV has more total Model entries than your game files. Something is wrong. Please double-check your file selections and settings. The Model entries read from the CSV have NOT been loaded."
                )
            if model_check == 0:
                logger.warning("Loading Model List from CSV")
        ...
        logger.exception("poke_edit_data failure")
        return poke_edit_data
...
def user_prompt_load_CSV(poke_edit_data, target):
    ...
    poke_edit_data.csv_pokemon_list_path = askopenfilename(
    ...
    for row in poke_edit_data.master_list_csv:
        if row[4] != "" and row[4] != "NA":
            if row[4] >= 0 and len(row) == 5:
                temp = [
                    poke_edit_data.model_header[flags_offset],
                    poke_edit_data.model_header[flags_offset + 1],
                ]
                flags_offset += 2
            else:
                temp = ["", ""]
...
def load_game_cfg(poke_edit_data):
    ...
    game_cfg_path = askopenfilename(
        title="Select cfg file", defaultextension=".cfg", filetypes=[("config", ".cfg")]
    )
    ...
    cfg_desc = [
        ...
        "Sorted",
        "Max Species Index",
        "Names and Model File List CSV Path",
    ]
    ...
    poke_edit_data = load_GARC(
        poke_edit_data, poke_edit_data.levelup_path, "Levelup", poke_edit_data.game
    )
    poke_edit_data = load_GARC(
        poke_edit_data, poke_edit_data.evolution_path, "Evolution", poke_edit_data.game
    )
    poke_edit_data = load_GARC(
        poke_edit_data, poke_edit_data.model_path, "Model", poke_edit_data.game
    )
    poke_edit_data = load_names_from_CSV(poke_edit_data)
...
def save_game_cfg(poke_edit_data, game_set=""):
    ...
    game_cfg_path = asksaveasfilename(
        title="Select location to save cfg file",
