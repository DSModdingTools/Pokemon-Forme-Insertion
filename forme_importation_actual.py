    ...
    for row_number, row in enumerate(poke_edit_data.master_list_csv):
        ...
        if isinstance(row[2], int) and isinstance(row[3], int):
            ...
            if row[2] == row[3]:
                ...
                old_forme_pointer = from_little_bytes_int(
                    poke_edit_data.personal[row[2]][0x1C:0x1E]
                )

                # if there are more personal files
                if old_forme_pointer != 0 and forme_count > 1:
                    # update with new pointer
                    poke_edit_data.personal[row[2]][0x1C:0x1E] = from_int_little_bytes(
                        forme_index, 0x2
                    )

                    # then for each of the alts, update the pointer, then move all 3 files to the new array
                    for x in range(forme_count - 1):
                        poke_edit_data.personal[old_forme_pointer + x][0x1C:0x1E] = (
                            from_int_little_bytes(forme_index, 0x2)
                        )

                        temp_personal[forme_index] = poke_edit_data.personal[
                            old_forme_pointer + x
                        ]
                        temp_levelup[forme_index] = poke_edit_data.levelup[
                            old_forme_pointer + x
                        ]
                        temp_evolution[forme_index] = poke_edit_data.evolution[
                            old_forme_pointer + x
                        ]

                        # base forme, old forme index, new forme index
                        repoint_alts_array.append(
                            [row[2], old_forme_pointer + x, forme_index]
                        )
...
def check_adding_without_models_works(poke_edit_data, base_form_index, new_forme_count):
    ...
    temp_pointer = from_little_bytes_int(
    ...
    if temp_model_count >= temp_forme_count_from_personal_file_count + new_forme_count:
        ...
        if (
            explicit_forme_count
            >= temp_forme_count_from_personal_file_count + new_forme_count
        ):
        ...
        print(
            ...
            + " model files and "
            + str(temp_forme_count_from_personal_file_count)
            + " Personal files, aborting."
        )
...
def add_new_forme_execute(
    ...
    new_forme_count,
    model_source_index,
    personal_source_index,
    levelup_source_index,
    evolution_source_index,
    def_model,
    ...
    for index, pokemon in enumerate(poke_edit_data.personal):
        ...
        if cur_forme_pointer >= new_forme_pointer:
            pokemon[0x1C:0x1E] = from_int_little_bytes(
                cur_forme_pointer + new_forme_count, 0x2
            )

    for x in range(new_forme_count):
        poke_edit_data.personal.insert(
            new_forme_pointer + x, poke_edit_data.personal[personal_source_index]
        )
        poke_edit_data.levelup.insert(
            new_forme_pointer + x, poke_edit_data.levelup[levelup_source_index]
        )
        poke_edit_data.evolution.insert(
            new_forme_pointer + x, poke_edit_data.evolution[evolution_source_index]
        )
    ...
    for index in existing_formes_array:
        ...
        poke_edit_data.personal[index][0x1C:0x1E] = from_int_little_bytes(
            use_pointer, 0x2
        )
    ...
    if skip_model_insertion:
        ...
        total_previous_models = poke_edit_data.model_header[
            4 * (base_form_index - 1) + 2
        ] + from_little_bytes_int(
            poke_edit_data.model_header[
                4 * (base_form_index - 1) : 4 * (base_form_index - 1) + 2
            ]
        )
        ...
        if def_model:
            model_source_index = (
                poke_edit_data.model_header[4 * (base_form_index - 1) + 1] * 256
                + poke_edit_data.model_header[4 * (base_form_index - 1) + 0]
            )
        ...
        while True:
            if poke_edit_data.model_header[
                start_of_byte_flag_table : start_of_byte_flag_table + 2
            ] != [0x0, 0x0]:
        ...
        model_source_flags = poke_edit_data.model_header[
            model_source_flag_offset : model_source_flag_offset + 2
        ]
        ...
        target_bitflag_offset = (
            2 * (total_previous_models) + start_of_byte_flag_table + 1
        )

        for x in range(new_forme_count):
            poke_edit_data.model_header.insert(
                target_bitflag_offset, model_source_flags[1]
            )
            poke_edit_data.model_header.insert(
