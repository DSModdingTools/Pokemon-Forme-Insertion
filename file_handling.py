...
def personal_file_update(poke_edit_data, target_index, total_formes, start_location):
    ...
    if start_location > 0:
        poke_edit_data.personal[target_index][0x1C:0x1E] = from_int_little_bytes(
            start_location, 0x2
        )
...
def update_csv_after_changes(
    ...
    if inserted_bool:
        ...
        for offset in range(new_forme_count):
            poke_edit_data.master_list_csv[first_missing_personal_line + offset][3] = (
        ...
        for offset in range(new_forme_count):
            ...
            poke_edit_data.master_list_csv.insert(
                csv_insertion_point + offset,
                [
                    base_species_name,
                    len(working_indices) + offset,
                    base_form_index,
                    start_location + offset,
                    0,
                    model_source_flags[0],
                    model_source_flags[1],
                ],
            )
        ...
        for offset in range(1, len(poke_edit_data.master_list_csv)):
            ...
            try:
                ...
                if (
                    poke_edit_data.master_list_csv[offset][3] >= start_location
                    and poke_edit_data.master_list_csv[offset][2] > base_form_index
                ):
