...
def entire_of_column(input_table, column_number, allow_multiple=True):
    ...
    last_element = ""
    for rows in input_table:
        if not (rows[column_number] in {"", "NA"}) and (
            allow_multiple or rows[column_number] != last_element
        ):
...
def find_rows_with_column_matching(
    input_table, column_number, search_term, only_one=False
):
    found_table = []
...
def file_namer(folder, index, length, poke_edit_data, file_prefix=""):
    ...
    return (
        os.path.join(folder, file_prefix + str(index).zfill(length))
        ...
        order_table.append(
            to_sort_table.pop(
                find_rows_with_column_matching(
                    to_sort_table, 1, max_of_column(to_sort_table, 1)
                )[0]
            )
        )
