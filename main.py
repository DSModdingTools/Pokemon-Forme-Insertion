...
def pre_check(poke_edit_data):
    ...
    try:
        if evolution_bool.get() or evolution_combobox.get() == "":
            ...
            evolution_source_index = int(
                poke_edit_data.master_formes_list.index(
    ...
    if skip_model_creation_bool.get():
        poke_edit_data, skip_model_insertion, update_forme_count = (
    ...
    # get the model index
    if model_bool.get():
        model_source_index_row = find_rows_with_column_matching(
            poke_edit_data.master_list_csv, 3, personal_source_index
        )[0]
    else:
        model_source_index_row = find_rows_with_column_matching(
            poke_edit_data.master_list_csv, 4, model_source_index
        )[0]
...
cfg_load = Button(
    ...
    text="Load CFG & CSV",
    command=lambda: [
        load_game_cfg(poke_edit_data),
        set_games_checklist(poke_edit_data.game),
...
load_pokelist_csv_button = Button(
    ...
    text="(Re)Load CSV",
    command=lambda: [
        user_prompt_load_CSV(poke_edit_data, "Pokemon Names and Files"),
        update_non_model_lists(poke_edit_data),
        update_model_list_for_box(poke_edit_data),
    ],
    height=2,
...
save_pokelist_csv_button = Button(
    ...
    command=lambda: [
        user_prompt_write_CSV(
            poke_edit_data, "Pokemon Names and Files", games_temp.get()
        ),
        update_non_model_lists(poke_edit_data),
        update_model_list_for_box(poke_edit_data),
    ],
